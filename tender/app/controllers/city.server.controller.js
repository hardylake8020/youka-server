...
var appDb = require('../../../libraries/mongoose').appDb,
  Province = appDb.model('Province');
City = appDb.model('City');
...
//  var provinces = {};
//  async.auto({
//    findProvinces: function (autoCallback) {
//      City.find({level: 1}).exec(function (err, result) {
//        if (err) {
...
//      });
//    },
//    findCities: ['findProvinces', function (autoCallback, result) {
//      var provinceArray = result.findProvinces || [];
//      async.eachSeries(provinceArray, function (item, provinceCallback) {
//
//        City.find({parent_city: item._id}).exec(function (err, cities) {
//          if (err) {
//            return provinceCallback({err: err});
//          }
//          var newCities = [];
//          async.each(cities, function (cityItem, cityCallback) {
//            City.find({parent_city: cityItem._id}).exec(function (err, regions) {
//              newCities.push({name: cityItem.name, region: regions.map(function (regionItem) {
//                return regionItem.name
//              })});
//              return cityCallback();
//            });
//          }, function (err) {
//            provinces[item.name] = newCities;
//            return provinceCallback();
//          });
//
//        });
//
...
exports.setCities = function (req, res, next) {
  //return res.send('skip function');
  ...
  Province.count({}, function (err, count) {
    ...
    async.each(provinceArray, function (provinceItem, provinceCallback) {
      var newProvince = new Province({
        ...
        cities: provinceItem.cities
      });

      newProvince.save(function (err) {
        if (err) {
          return provinceCallback({err: err});
        }
        return provinceCallback();
...
exports.getCities = function (req, res, next) {
  var provinceObject = {};
  async.auto({
    findProvince: function (provinceCallback) {
      Province.find({}).exec(function (err, provinces) {
        if (err) {
          return provinceCallback({err: error.system.db_error});
        }
        return provinceCallback(null, provinces);
      });
    },
    combine: ['findProvince', function (combineCallback, result) {
      var provinces = result.findProvince || [];
      async.each(provinces, function (provinceItem, provinceCallback) {
        provinceObject[provinceItem.name] = provinceItem.cities;
        return provinceCallback();
      }, function (err) {
        return combineCallback();
      });
    }]
  }, function (err, result) {
    if (err) {
