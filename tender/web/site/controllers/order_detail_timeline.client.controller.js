 ...
 */
tender.controller('OrderDetailTimelineController',
  ['$state', '$scope', '$stateParams', '$timeout', 'OrderHelper', 'config', 'HttpTender',
    function ($state, $scope, $stateParams, $timeout, OrderHelper, config, HttpTender) {
      ...
      function getTenderDetail() {
        HttpTender.getTenderByTenderId($scope, {
          ...
          if (data && data.order)
            getOrderEvent(data.order._id)
        });
      }



      function getOrderEvent(orderId) {
