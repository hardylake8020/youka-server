...
/*
 * 测试模块：api修改分段
 * 测试目标：第三方通过接口修改一段司机分配（包括已分配和未分配段）
 * 所有入口：api修改分段入口
 * */
...
describe('Test For Api Order Controller::', function () {
  var user_A, company_A, group_name_A, signature_A, timestamp_A,driver_1, driver_2, driver_3,
    order_A, order_B, order_C, order_D, assign_info_A, assign_info_B, assign_info_C, assign_info_blank;
  var deepCopy;
  ...
  before(function (done) {
    ...
    driver_3 = {username: '13564083674', password: '111111'};
    ...
    assign_info_B = deepCopy(assign_info_A);
    assign_info_B.driver_username = driver_2.username;
    assign_info_C = deepCopy(assign_info_A);
    assign_info_C.driver_username = driver_3.username;

    assign_info_blank = deepCopy(assign_info_A);
    delete assign_info_blank.driver_username;
    delete assign_info_blank.driver_id;
  ...
  var userA, tokenA, companyA, driver1, tokenD1, driver2,tokenD2, driver3,tokenD3, companyKey1, driverOrder1, driverOrder2;
  ...
  describe('Test for api modify order assign', function () {
    var assign_info_id;

    //orderA 只有一个分段
    it('should be error when modify order assign at unAssigned status', function (done) {
      OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, null, assign_info_blank,function (err, result) {
        result.err.type.should.equal('empty_assign_info_id');
        done();
      });
    });
    it('should be error when modify order assign at unAssigned status', function (done) {
      OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, '123456', assign_info_blank,function (err, result) {
        result.err.type.should.equal('empty_assign_info');
        done();
      });
    });
    it('should be error when modify order assign at unAssigned status', function (done) {
      OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, '123456', assign_info_A,function (err, result) {
        result.err.type.should.equal('assign_info_can_not_modify');
        done();
      });
    });

    it('should be success when assign orderA to driver1', function (done) {
      assign_info_A.driver_username = driver1.username;
      assign_info_A.type = 'driver';

      OrderWebAPI.apiAssignOrderToDriver(signature_A, order_A.order_number, companyA._id, timestamp_A, [assign_info_A], function (err, result) {
        result.success.should.equal(true);
        Order.findOne({
    ...
    it('should be error when modify orderA assign with invalid assign id', function (done) {
      OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, '123456', assign_info_B, function (err, result) {
        result.err.type.should.equal('assign_info_not_exist');
        done();
      });
    });

    it('should be error when modify orderA assign with invalid assign type', function (done) {
      Order.findOne(
        {
          'order_details.order_number': order_A.order_number,
          execute_driver: {$exists: false}
          ...
          order.save(function (err, saveOrder) {
            OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, assign_info_id, assign_info_B, function (err, result) {
    ...
    it('should be success when modify orderA assign with same assign info', function (done) {
      Order.findOne({
        'order_details.order_number': order_A.order_number,
        execute_driver: {$exists: false}
      }, function (err, order) {
        var assign_id = order.assigned_infos[0]._id.toString();
        OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, assign_id, assign_info_A, function (err, result) {
          result.err.type.should.equal('assign_same_driver');
          done();
        });
      });
    });

    it('should be success when modify orderA assign with valid assign info', function (done) {
      Order.findOne({
        'order_details.order_number': order_A.order_number,
        execute_driver: {$exists: false}
      }, function (err, order) {
        var assign_id = order.assigned_infos[0]._id.toString();
        OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_A.order_number, assign_id, assign_info_B, function (err, result) {
          result.success.should.equal(true);
          Order.findOne({
            'order_details.order_number': order_A.order_number,
            execute_driver: {$exists: false}
            ...
            Order.findOne({
              ...
              order.execute_drivers.length.should.equal(1);
              order.execute_driver.toString().should.equal(driver2._id.toString());
              done();
            });
          });
        });
    ...
    //order_B 有两个分段，都有实际分配driver1 & driver2
    it('should be success when assign orderB to driver1 and driver2', function (done) {
      assign_info_A.driver_username = driver1.username;
      assign_info_A.type = 'driver';
      assign_info_B.driver_username = driver2.username;
      assign_info_B.type = 'driver';

      OrderWebAPI.apiAssignOrderToDriver(signature_A, order_B.order_number, companyA._id, timestamp_A, [assign_info_A, assign_info_B], function (err, result) {
        result.success.should.equal(true);
        Order.findOne({
    ...
    it('should be error when modify orderB assign with pickup sub order', function (done) {
      Order.findOne({
        'order_details.order_number': order_B.order_number,
        execute_driver: {$exists: false}
      }, function (err, order) {
        ...
        OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_B.order_number, assign_id, assign_info_B, function (err, result) {
          result.err.type.should.equal('assign_info_can_not_modify');
          done();
        });
    ...
    it('should be success when modify orderB assign with valid assign info', function (done) {
      Order.findOne({
        'order_details.order_number': order_B.order_number,
        execute_driver: {$exists: false}
      }, function (err, order) {
        ...
        OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_B.order_number, assign_id, assign_info_C, function (err, result) {
          result.success.should.equal(true);
          Order.findOne({
            'order_details.order_number': order_B.order_number,
            execute_driver: {$exists: false}
            ...
            order.total_assign_count.should.equal(2);
            order.assigned_count.should.equal(2);
            order.execute_drivers.length.should.equal(2);
            if (order.execute_drivers[0].username === driver1.username) {
              order.execute_drivers[1].username.should.equal(driver3.username);
            }
            else {
              order.execute_drivers[0].username.should.equal(driver3.username);
            }
            ...
            Order.findOne({
              ...
              Order.findOne({
                'order_details.order_number': order_B.order_number,
                execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
              }, function (err, order) {
                order.delete_status.should.equal(false);
                order.execute_drivers.length.should.equal(1);
                order.execute_drivers[0].username.should.equal(driver3.username);
                order.status.should.equal('unPickupSigned');
                done();
    ...
    it ('should be success when driver1 delivery orderB', function (done) {
      Order.findOne({
        'order_details.order_number': order_B.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver1._id.toString())
      }, function (err, driverOrder) {
    ...
    it ('should be success when driver3 pickup orderB', function (done) {
      Order.findOne({
        'order_details.order_number': order_B.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
      }, function (err, driverOrder) {
        driverOrder.type.should.equal('driver');
        TransportEventWebAPI.uploadEvent(tokenD3, driverOrder._id.toString(), 'pickup', true, function (err, result) {
    ...
    it ('should be success when driver3 delivery orderB', function (done) {
      Order.findOne({
        'order_details.order_number': order_B.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
      }, function (err, driverOrder) {
        driverOrder.type.should.equal('driver');
        TransportEventWebAPI.uploadEvent(tokenD3, driverOrder._id.toString(), 'delivery', true, function (err, result) {
    ...
    it('should be error when modify order assign at completed status', function (done) {
      OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_B.order_number, '123456', assign_info_A, function (err, result) {
        result.err.type.should.equal('assign_info_can_not_modify');
        done();
      });
    });
  ...
  describe('Test for api modify orderC assign with blank assign info before driver1 delivery', function () {
    it('should be success when assign orderC to driver1 and blank ', function (done) {
      assign_info_A.driver_username = driver1.username;
      assign_info_A.type = 'driver';
      delete assign_info_B.driver_username;
      delete assign_info_B.type;

      OrderWebAPI.apiAssignOrderToDriver(signature_A, order_C.order_number, companyA._id, timestamp_A, [assign_info_A, assign_info_B], function (err, result) {
        result.success.should.equal(true);
        Order.findOne({
    ...
    it('should be success when modify orderC assign with blank assign', function (done) {
      Order.findOne({
        'order_details.order_number': order_C.order_number,
        execute_driver: {$exists: false}
      }, function (err, order) {
        ...
        OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_C.order_number, assign_id, assign_info_C, function (err, result) {
          result.success.should.equal(true);
          Order.findOne({
            'order_details.order_number': order_C.order_number,
            execute_driver: {$exists: false}
    ...
    it ('should be success when driver1 delivery orderC', function (done) {
      Order.findOne({
        'order_details.order_number': order_C.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver1._id.toString())
      }, function (err, driverOrder) {
    ...
    it ('should be success when driver3 pickup orderC', function (done) {
      Order.findOne({
        'order_details.order_number': order_C.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
      }, function (err, driverOrder) {
    ...
    it ('should be success when driver3 delivery orderC', function (done) {
      Order.findOne({
        'order_details.order_number': order_C.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
      }, function (err, driverOrder) {
  ...
  describe('Test for api delete orderD assign with blank assign info after driver1 delivery', function () {
    it('should be success when assign orderD to driver1 and blank ', function (done) {
      ...
      OrderWebAPI.apiAssignOrderToDriver(signature_A, order_D.order_number, companyA._id, timestamp_A, [assign_info_A, assign_info_B], function (err, result) {
        result.success.should.equal(true);
        Order.findOne({
    ...
    it('should be success when modify orderD assign with blank assign', function (done) {
      Order.findOne({
        'order_details.order_number': order_D.order_number,
        execute_driver: {$exists: false}
      }, function (err, order) {
        ...
        OrderWebAPI.apiModifyOrderAssign(signature_A, companyA._id, timestamp_A, order_D.order_number, assign_id, assign_info_C, function (err, result) {
          result.success.should.equal(true);
          Order.findOne({
            'order_details.order_number': order_D.order_number,
            execute_driver: {$exists: false}
    ...
    it ('should be success when driver3 pickup orderD', function (done) {
      Order.findOne({
        'order_details.order_number': order_D.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
      }, function (err, driverOrder) {
    ...
    it ('should be success when driver3 delivery orderD', function (done) {
      Order.findOne({
        'order_details.order_number': order_D.order_number,
        execute_driver: new mongoose.Types.ObjectId(driver3._id.toString())
      }, function (err, driverOrder) {
