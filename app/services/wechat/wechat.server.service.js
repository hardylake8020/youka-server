...
var path = require('path'),
  ...
  qiniuToken = require('../../services/qiniu_token'),
...
function refreshTokenAndTicket(callback) {
  ...
  https.get(token_url, function (tokenRes) {
    tokenRes.on('data', function (tokenResult) {
      ...
      var ticket_url = config.wx_url + 'ticket/getticket?access_token=' + _token + '&type=jsapi';
...
exports.getAccessTokenAndOpenIdByCode = function (code, callback) {
  var url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' + config.wx_appid + '&secret=' + config.wx_secret + '&code=' + code + '&grant_type=authorization_code';
  superagent.get(url)
    .end(function (err, result) {
      return callback(err, JSON.parse(result.text));
    });
...
exports.createVerifyCode = function (phone, callback) {
...
exports.getVerifyCode = function (verifyCodeId, code, callback) {
  SmsVerify.findOne({_id: verifyCodeId, code: code}, function (err, verify) {
    if (err) {
      return callback({err: {type: 'internal_system_error'}});
    }

    if (!verify) {
      return callback({err: {type: 'invalid_code'}});
    }

    return callback(null, verify);
  });
};
exports.unbindObject = function (currentObject, callback) {
  if (!currentObject) {
    return callback({err: 'invalid_params'});
  }

  currentObject.wechat_openid = '';
  currentObject.wechat_profile = {};
  currentObject.markModified('wechat_profile');

  currentObject.save(function (err, saveObject) {
    if (err || !saveObject) {
      return callback({err: 'internal_system_error'});
    }
    return callback(null, saveObject);
  });
};
