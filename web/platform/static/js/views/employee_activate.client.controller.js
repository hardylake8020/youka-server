...
function RegistFramework(container) {
  ...
  (function () {
    ...
    Element.Register.Submit.click(function () {
      ...
      $.ajax({
        ...
        type: 'post',
        url: '/user/employee/activate/'+$('#username').val()+'?password='+$('#password').val(),
        dataType: 'json'
      ...
      $.ajax({
        ...
        if (result.err) {
          switch (result.err.type) {
            ...
            case 'invalid_password': {
              alert("无效密码设置，请输入至少6位数密码!");
              break;
            }
            case 'account_exist': {
              alert("您的邮箱已经注册，不能重新注册，请使用新的邮箱注册或者直接登录！");
              break;
            }
            default :
            case 'internal_system_error': {
  ...
  function RegisterValid() {
    ...
    if (!Element.Register.EmailBox.hasClass('error')
      && !Element.Register.Password.hasClass('error')
      && !Element.Register.PasswordTwice.hasClass('error')
      && ReadMeNullCheck())
      return true;
    else
      return false;
